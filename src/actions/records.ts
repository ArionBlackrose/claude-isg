...
export async function createRecord(input: unknown): Promise<ActionResult> {
  ...
  const kategoriError = getGeneralCreationError(selectedTraining?.kategori);
  if (kategoriError) {
...
/** `createRecords`/`createUyariRecords` arasında paylaşılan asıl toplu
 * ekleme mantığı. `mode` bu action'ın hangi kategori kuralını uygulayacağını
 ...
 *
 * Saha Eğitimi kayıtları BU action'dan geçmez — kendi ayrı akışı var (bkz.
 * src/actions/saha-egitimi.ts), çünkü form alanları (sonuç/dosya no/not
 * yerine kataloglu başlık seçimi) yeterince farklılaştığı için bu genel
 * mekanizmaya zorla uydurmak yerine ayrı bir action daha net oldu. */
...
export async function createRecords(input: unknown): Promise<RecordsBatchResult> {
  return createRecordsInternal(input, 'general');
}
...
export async function importRecordsFromExcel(
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const kategoriError = getGeneralCreationError(trainingMatch.kategori);
    if (kategoriError) {
      skipped.push({ row: rowNo, reason: `"${trainingMatch.ad}": ${kategoriError}` });
      continue;
    }
